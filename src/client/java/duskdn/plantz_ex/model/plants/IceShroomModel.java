...
import duskdn.plantz_ex.animation.plants.IceShroomAnimation;
...
public class IceShroomModel extends PazPlantModel {
	...
	public IceShroomModel(ModelPart root) {
		super(
			IceShroomAnimation.init.bake(root),
			IceShroomAnimation.idle.bake(root),
			IceShroomAnimation.action.bake(root),
			IceShroomAnimation.sleep.bake(root),
				null,
			root
		);
	...
	public void setupAnim(@NotNull PlantRenderState state) {
		...
		this.body.yRot += state.yRot * (float) (Math.PI / 180.0) / 2;
	}
}